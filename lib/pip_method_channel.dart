...
class MethodChannelPip extends PipPlatform {
  ...
  Future<void> _handleMethod(MethodCall call) async {
    try {
      ...
      } else if (call.method == 'pipViewSizeDebug') {
        final jsonMap = Map<String, dynamic>.from(call.arguments as Map);
        final w = (jsonMap['width'] as num).toDouble();
        final h = (jsonMap['height'] as num).toDouble();
        PipPlatform.onPipViewSizeReported?.call(w, h);
      }
    } catch (e) {
      assert(false, 'pip channel handler error: $e');
    }
  }
