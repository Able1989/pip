...
/// iOS：`setup` 完成后原生回传 [PipView] 的 `bounds.size`（逻辑点）。
typedef PipViewSizeCallback = void Function(double width, double height);
...
class PipOptions {
  ...
  /// iOS：宿主 `AVPictureInPictureVideoCallViewController.view`（即 [PipView] 的
  /// `superview`）背景色，与 Flutter [Color.value] 相同（0xAARRGGBB）。传 `0` 为全透明。
  /// 不传则由原生按「透明宿主」处理。
  final int? iosPipHostBackgroundArgb;

  /// iOS：为 `true` 时 PipView 的 sample buffer 填全透明像素（适合语音仅中间小卡片），
...
abstract class PipPlatform extends PlatformInterface {
  ...
  /// 若不为 null，iOS 原生在 `setup` 成功后会回传 PipView 宽高（见 `pipViewSizeDebug`）。
  static PipViewSizeCallback? onPipViewSizeReported;
